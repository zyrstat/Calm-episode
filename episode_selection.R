...
choosep <- function(csysname,sysepname,sit,n12,bbb1)
{
  ...
  #  sit <- "a"#输入站点
  csys <- read.csv(paste0(path6,csysname), header = TRUE)##agwyearseason,agw2017xia
  site=csys$loca
  ...
  write.csv(sys_ep_before,file=paste0('/Users/zyr/Documents/PM2.5/datas/data_BJ_TS_BD/episodes_before/',sysepname,'_before.csv'))
  
  sys_ep_after=cbind(da_s[sec_after,],ep_id_after)
  sys_ep_after=cbind(sys_ep_after,orde_after)
  write.csv(sys_ep_after,file=paste0('/Users/zyr/Documents/PM2.5/datas/data_BJ_TS_BD/episodes_after/',sysepname,'_after.csv'))
...
for(k in 1:nrow(csnames))
{
  ...
  n12=c(0,0,0)
  da_cs <-read.csv(paste0(path5,prefix,sites[1],sites[2],sites[3],'_cleaned.csv'),header = TRUE) #da_agw
  ...
  for(j in 1:length(sitn))
  {
    ...
    daNS_s=read.csv(paste0(path4,sNSn[j]), header = TRUE)#daNS_a
