  ...
  for(i in 1:length(seasons))
  {
    for(pp in 1:length(pps))
    {
      ggplot(ad_PNS[ad_PNS$Pollutant==pps[pp]&ad_PNS$site==rnames[u]&ad_PNS$season==seasons[i],],aes(x=orde, y=adjusted,group=factor(year)))+
        ...
        facet_wrap(.~ factor(subtitle),ncol=6, scales = "fixed") + 
        labs(x = "Hour", y = paste0('Average Growth'))+mytheme
      ggsave(paste0(pathsavegp,pps[pp],rnames[u],seasons[i],'adjustedfacet.png'),units="in",width=7, height=1.73, dpi=300)
...
vs_PM=read.csv("/Users/zyr/Documents/PM2.5/datas/data_BJ_TS_BD/form_adjusted/PM2.5selectedvariables.csv",stringsAsFactors = F)
vs_NO=read.csv("/Users/zyr/Documents/PM2.5/datas/data_BJ_TS_BD/form_adjusted/NO2selectedvariables.csv",stringsAsFactors = F)
vs_SO=read.csv("/Users/zyr/Documents/PM2.5/datas/data_BJ_TS_BD/form_adjusted/SO2selectedvariables.csv",stringsAsFactors = F)
...
print(xtable(n_delete[,c(15,1:10)],
