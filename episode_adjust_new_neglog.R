...
set.seed(0)
pathsavegp="/Users/zyr/Documents/PM2.5/datas/data_BJ_TS_BD/glsplotneglog/"
...
for(u in 1:length(rnames))
{
  ...
  for(i in 1:length(seasons))
  {
    ...
    for(j in 1:length(years))
    {
      ...
      # step(lm(neglog(pollutant)~TEMP+PRES+DEWP+blh+HUMI+WSPM+uwind+vwind+uIws+vIws+Iwu+Iwv+IIwu+IIwv+
      ...
      dataaa1_cv <- cv.gglasso(x=as.matrix(dataaa1_feature),y=neglog(dataaa1_dummy$pollutant),
                               group=dataaa1_group,loss="ls",
                               pred.loss="L2", nfolds=5)
      
      dataaa1_try <- gglasso(x=as.matrix(dataaa1_feature),y=neglog(dataaa1_dummy$pollutant),group=dataaa1_group,loss="ls")
    ...
    for(k in 1:length(years))
    {
      ...
      M1=lm(neglog(pollutant) ~ 1,data=datas1[[k]])
      ...
      # pbsytest(neglog(pollutant) ~ factor(orde) + TEMP + PRES +
      ...
      # pwartest(neglog(pollutant) ~ factor(orde) + TEMP + PRES +
      #            DEWP + blh + Iwu + Iwv + epfNen + WSPM, data = datas1[[k]],index=c("ep_id","orde"))
      # #Modified BNF–Durbin–Watson Test for AR(1) disturbances in panel models.
      # pbnftest(neglog(pollutant) ~ factor(orde) + TEMP + PRES +
      #            DEWP + blh + Iwu + Iwv + epfNen + WSPM, data = datas1[[k]],index=c("ep_id","orde"),model="pooling")
      # #equivalent to lm
      # Model1=panelAR(neglog(pollutant) ~ factor(orde),data = datas1[[k]],
      ...
      #all(datas1[[k]]$residuals==neglog(datas1[[k]]$pollutant)-datas1[[k]]$fitted)
      ...
      datas1[[k]]$fitted=inverse_neglog(datas1[[k]]$fitted)
     ...
     #和 episode_adjust_new.R中的计算等价 
      ...
      varx_add=diag(var_ob)/length(years)^2
      ...
      mean_or_ad[[k]]$adjusted=inverse_neglog(adjusted$fit)
      mean_or_ad[[k]]$se=exp(abs(adjusted$fit))*sqrt((adjusted$se)^2+varx_add)
      #qnorm(1-0.05/2/(eplen-1))
