...
set.seed(0)
pathsavegp="/Users/zyr/Documents/PM2.5/datas/data_BJ_TS_BD/glsplotlog/"
...
for(u in 1:length(rnames))
{
  ...
  for(i in 1:length(seasons))
  {
    ...
    for(j in 1:length(years))
    {
      ...
      d$pollutant=log(d[,pollutant_sp])
      ...
      # rank_ob[which(!names(rank_ob)%in%elements[[j]])]=NA
    ...
    # rank_all=ranks[,-c((ncol(ranks)-eplen+3):ncol(ranks))]
    # rank_ge=data.frame(feature=colnames(rank_all),number=apply(!is.na(rank_all),2,sum),rank.m=apply(rank_all,2,mean,na.rm=T))
    # rank_ge=arrange(rank_ge,desc(number),rank.m)
    # name_rank=rank_ge$feature
