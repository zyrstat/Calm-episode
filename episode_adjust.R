...
for(u in 1:length(rnames))
{
  ...
  for(i in 1:length(seasons))
  {
    ...
    for(j in 1:length(years))
    {
      ...
      #   labs(x = "Time", y = 'PM2.5')
      # 
      # boxplot(PM2.5~orde, data=da)
      
      origin=da[which(da$orde==1),c('TEMP','PRES','DEWP','blh','HUMI','WSPM','uwind','vwind','uIws','vIws','Iwu','Iwv','IIwu','IIwv','PM2.5')]
      repe=c()
      for(k in 1:length(id_period))
      {
        repe=rbind(repe,matrix(unlist(rep(as.vector(origin[k,]),each=eplen)),nrow=eplen))
      }
      colnames(repe)=c('TEMP','PRES','DEWP','blh','HUMI','WSPM','uwind','vwind','uIws','vIws','Iwu','Iwv','IIwu','IIwv','PM2.5')
      
      dataa=as.matrix(da)  
      col_sub=which(colnames(dataa)%in%c('TEMP','PRES','DEWP','blh','HUMI','WSPM','uwind','vwind','uIws','vIws','Iwu','Iwv','IIwu','IIwv','PM2.5'))
      #after difference
      dataa[,col_sub]=dataa[,col_sub]-repe
      dataa=as.data.frame(dataa)

      # ggplot(dataa,aes(x=orde, y=PM2.5,group=factor(ep_id)))+
      #   geom_point(size=0.5)+geom_line()+stat_smooth(method="lm",se = F)+
      #   facet_wrap(.~ ep_id,ncol=8, scales = "fixed") + theme_bw()+
      #   labs(x = "Time", y = 'PM2.5')
      #delete the first point in each episode
      dataaa=dataa[1<dataa$orde,]
      # ggplot(dataaa,aes(x=orde, y=PM2.5,group=factor(ep_id)))+
      #   geom_point(size=0.5)+geom_line()+stat_smooth(method="lm",se = F)+
      #   facet_wrap(.~ ep_id,ncol=8, scales = "fixed") + theme_bw()+
      #   labs(x = "Time", y = 'PM2.5')
      # 
      # step(lm(PM2.5~TEMP+PRES+DEWP+blh+HUMI+WSPM+uwind+vwind+uIws+vIws+Iwu+Iwv+IIwu+IIwv+
      ...
      # #                 'epfpen48','epfNen48','orde_3','orde_4','orde_5','orde_6','orde_7')]
      # dataaa_feature=dataaa_dummy[,c('TEMP','PRES','DEWP','blh','HUMI','WSPM',
      #                                'Iwu','Iwv',
      #                                'epfNen',
      #                                'orde_3','orde_4','orde_5','orde_6','orde_7')]
      # dataaa_group=c(1:(ncol(dataaa_feature)-eplen+2),rep(ncol(dataaa_feature)-eplen+3,eplen-2))
      # dataaa_cv <- cv.gglasso(x=as.matrix(dataaa_feature),y=dataaa_dummy$PM2.5,
      #                         group=dataaa_group,loss="ls",
      #                         pred.loss="L2", nfolds=5)
      # 
      # # the coefficients at lambda = lambda.1se
      # coeff1=coef(dataaa_cv$gglasso.fit,s=dataaa_cv$lambda.1se)
      # elemen1=c(elemen1,names(coeff1[setdiff(which(coeff1!=0),c(1,(length(coeff1)-eplen+3):length(coeff1))),]))
      # # #fit group lasso
      # # dataaa_try <- gglasso(x=as.matrix(dataaa_feature),y=dataaa_dummy$PM2.5,group=dataaa_group,loss="ls")
      # # print(dataaa_try)

      dataaa1=dataaa
      (id_delete=unique(dataaa1$ep_id[dataaa1$PM2.5<=-lower]))
      if(length(id_delete)>0)
        print(ggplot(dataaa1[which(dataaa1$ep_id%in%id_delete),],aes(x=orde, y=PM2.5,group=factor(ep_id)))+
        geom_point(size=0.5)+geom_line()+stat_smooth(method="lm",se = F)+
        facet_wrap(.~ ep_id,ncol=8, scales = "fixed") + theme_bw()+
        labs(x = "Time", y = 'PM2.5',title='Episodes deleted'))
      
      dataaa1=dataaa1[which(!dataaa1$ep_id%in%id_delete),]
      allls=c(allls,length(unique(dataaa1$ep_id)))
      # step(lm(log(lower+PM2.5)~TEMP+PRES+DEWP+blh+HUMI+WSPM+uwind+vwind+uIws+vIws+Iwu+Iwv+IIwu+IIwv+
      #           epfmINws+epfpn+epfNen+epfpen+pmf35+epfmINws48+epfpn48+epfpen48+epfNen48+
      #           factor(orde),dataaa1))
      ...
      dataaa1_feature=cbind(as.data.frame(scale(dataaa1_dummy[,c('TEMP','PRES','DEWP','blh','HUMI','WSPM',
                            ...
                            dataaa1_dummy[,c('orde_3','orde_4','orde_5','orde_6','orde_7')])
      dataaa1_group=c(1:(ncol(dataaa1_feature)-eplen+2),rep(ncol(dataaa1_feature)-eplen+3,eplen-2))
      dataaa1_cv <- cv.gglasso(x=as.matrix(dataaa1_feature),y=log(lower+dataaa1_dummy$PM2.5),
                               group=dataaa1_group,loss="ls",
                               pred.loss="L2", nfolds=5)
      
      dataaa1_try <- gglasso(x=as.matrix(dataaa1_feature),y=log(lower+dataaa1_dummy$PM2.5),group=dataaa1_group,loss="ls")
      ...
      # dataaa$residual=rstandard(lm(PM2.5 ~ TEMP + DEWP + blh + HUMI + WSPM + vwind + 
      #                                vIws + Iwu + IIwu + IIwv + epfpn + epfpen + factor(orde), 
      #                              data = dataaa))
      # dataaa1$residual=rstandard(lm(log(lower + PM2.5) ~ PRES + WSPM + vwind + vIws + 
      ...
      mm=as.data.frame(dataaa1 %>%
        ...
        summarise(TEMP = mean(TEMP),PRES=mean(PRES),DEWP=mean(DEWP),blh=mean(blh),HUMI = mean(HUMI),WSPM = mean(WSPM),
                  ...
                  epfpen48=mean(epfpen48),epfNen48=mean(epfNen48),PM2.5=mean(PM2.5)))
      mm=inner_join(mm,as.data.frame(dataaa1 %>%group_by(orde) %>%
           summarise(SE=sd(PM2.5)/sqrt(eplen-1))))
    ...
    for(k in 1:length(years))
    {
      ...
      # pbsytest(log(lower + PM2.5) ~ factor(orde) + TEMP + PRES +
      ...
      # pwartest(log(lower + PM2.5) ~ factor(orde) + TEMP + PRES +
      #            DEWP + blh + Iwu + Iwv + epfNen + WSPM, data = datas1[[k]],index=c("ep_id","orde"))
      # #Modified BNF–Durbin–Watson Test for AR(1) disturbances in panel models.
      # pbnftest(log(lower + PM2.5) ~ factor(orde) + TEMP + PRES +
      #            DEWP + blh + Iwu + Iwv + epfNen + WSPM, data = datas1[[k]],index=c("ep_id","orde"),model="pooling")
      # #equivalent to lm
      # Model1=panelAR(log(lower + PM2.5) ~ factor(orde),data = datas1[[k]],
      ...
      # out=panelAR(log(lower + PM2.5) ~ factor(orde) + PRES + DEWP + 
      #               WSPM + uwind + vwind + Iwv + epfNen,data = datas1[[k]],
      #             panel.weight = "t-1", rhotype ="breg",autoCorr = "ar1",
      #             panelCorrMethod = "none", panelVar="ep_id", timeVar="orde")
      modeAR1[[k]]=out
      ...
      #all(datas1[[k]]$residuals==log(lower + datas1[[k]]$PM2.5)-datas1[[k]]$fitted)
      datas1[[k]]$fitted=exp(datas1[[k]]$fitted)-lower
      datas1[[k]]$residuals=datas1[[k]]$PM2.5-datas1[[k]]$fitted

      # summary(out)$rho 
      # summary(out)$Sigma # panel covariances

      #fitted
      ggplot(datas1[[k]],aes(x=orde, y=PM2.5,group=factor(ep_id)))+
      ...
      mean_or_ad[[k]]=rbind(data.frame(orde=1,TEMP=0,PRES=0,DEWP=0,blh=0,HUMI=0,WSPM=0,
                          ...
                          PM2.5=0,SE=0,year=years[k],adjusted=0,ad.lb=0,ad.ub=0,se=0),mean_or_ad[[k]])
    ...
    ggplot(adjust_ba,aes(x=orde, y=adjusted,group=factor(year)))+
      ...
      geom_line(aes(x=orde, y=PM2.5),size=0.4,alpha=1)+
      facet_wrap(.~ year,ncol=3, scales = "fixed") + 
      labs(x = "Time", y = 'Difference of PM2.5')+mytheme
  ...
  ggplot(adjust_bas[adjust_bas$orde==eplen,],aes(x=factor(year),y=adjusted/(eplen-1),fill=season))+
    ...
    labs(x = 'Year',y = 'Growth rate of PM2.5 in six hours', title = paste0('Growth rate of PM2.5 in six hours for ',rnames[u]))
  ggsave(paste0(pathsavegp,rnames[u],'adjusted.png'),units="in",width=5, height=4, dpi=300)
