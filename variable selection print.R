...
for(u in 1:length(rnames))
{
  ...
  for(i in 1:length(seasons))
  {
    ...
    print_need=v_rank_season[,c(1:8,27:29)]
    ...
    #       include.rownames =F)
    print(xtable(apply(v_rank_season[,c(1:8,27:29)],2,as.character),
                 caption=paste0(pollutant_sp,' ',rnames[u],seasons[i])),
          include.rownames =F)
    ...
    ggplot(select_try,aes(x=num,y=R2))+geom_line(aes(color=factor(year),group=factor(year)))+
      ...
      scale_x_continuous(breaks=1:(length(candidate)+1),labels=paste0(sapply(element_order,convert_variables),'\n(',
...
for(ss in seasons)
{
  print(paste(apply(part_pm[part_pm$season==ss,paste0('V',1:(length(candidate)+1))],2,sum),collapse='&'))
}
...
print(sort(round(apply(num_print[1:4,1:10],2,sum,na.rm=T)/16,2),decreasing = T))
ord1=order(round(apply(num_print[1:4,1:10],2,sum,na.rm=T)/16,2),decreasing = T)
print(sort(round(apply(num_print[5:8,1:10],2,sum,na.rm=T)/16,2),decreasing = T))
ord2=order(round(apply(num_print[5:8,1:10],2,sum,na.rm=T)/16,2),decreasing = T)
print(sort(round(apply(num_print[9:12,1:10],2,sum,na.rm=T)/16,2),decreasing = T))
ord3=order(round(apply(num_print[9:12,1:10],2,sum,na.rm=T)/16,2),decreasing = T)

num_print[1:(length(pollutant_names)*length(seasons)),1:10]=round(num_print[1:(length(pollutant_names)*length(seasons)),1:10]/4,2)
num_print[nrow(num_print),1:10]=round(num_print[nrow(num_print),1:10]/(4*length(pollutant_names)*length(seasons)),2)
